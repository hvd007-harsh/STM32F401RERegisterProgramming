...
unsigned char ArrayData[30], ReceivedData1[5], ReceivedData2[5], ArrRead1[5] = {0xD3, 0x02, 0x03, 0xAD}, ArrRead2[5] = {0x63, 0x6F, 0x66, 0xAC}; // ArrRead1[5]={0xA3,0xE7,0xF1,0xAC},ArrRead2[5]={0x23,0xFF,0x0C,0xAD};
...
void Serial_ISR(void) interrupt 4
{
	...
	if (RI)
	{
		...
		k++;
		ArrayData[k] = uart0_receive_data;
	...
	while (1)
	{
		...
		if (status == MI_OK)
		{
				 ...
				 (str[3] == ArrRead2[3])))
			{
				...
				if ((str[0] == CardAUTH[0]) && (str[1] == CardAUTH[1]) && (str[2] == CardAUTH[2]) && (str[3] == CardAUTH[3]))
				{
					OverloadRetry = 0;
					if (flag == 0)
					{
